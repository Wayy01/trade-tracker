...
import { formatMonthLabel, gridDaysForMonth, isoDate } from "@/lib/dates";
      ...
      {months.map((m) => {
        ...
        return (
          <div key={m.toISOString()} className="flex flex-col">
            ...
            <div className="grid grid-cols-7 gap-1 pb-1 text-center text-[9px] font-medium uppercase tracking-wider text-muted-foreground/70">
              {WEEKDAYS.map((d, i) => (
                <div key={i}>{d}</div>
              ))}
            </div>
